    ...
    <div class="card-body">
        <div class="table-responsive list-table-wrapper">
            ...
            <table id="team-list-table" class="table m-t-0 m-b-0 table-hover no-wrap contact-list" data-page-size="10">
                <thead>
                    <tr>
                        <th class="team_col_first_name {{ app()->getLocale() == 'persian_br' ? 'text-right' : 'text-left' }}"><a class="js-ajax-ux-request js-list-sorting"
                        ...
                        <th class="team_col_position {{ app()->getLocale() == 'persian_br' ? 'text-right' : 'text-left' }}"><a class="js-ajax-ux-request js-list-sorting" id="sort_position"
                        ...
                        <th class="team_col_role {{ app()->getLocale() == 'persian_br' ? 'text-right' : 'text-left' }}"><a class="js-ajax-ux-request js-list-sorting" id="sort_role_id"
                                href="javascript:void(0)"
                                data-url="{{ urlResource('/team?action=sort&orderby=role_id&sortorder=asc') }}">{{ cleanLang(__('lang.role')) }}<span
                                    class="sorting-icons"><i class="ti-arrows-vertical"></i></span>
                        </th>
                        @endif
                        <th class="team_col_email {{ app()->getLocale() == 'persian_br' ? 'text-right' : 'text-left' }}"><a class="js-ajax-ux-request js-list-sorting" id="sort_email"
                                href="javascript:void(0)"
                                data-url="{{ urlResource('/team?action=sort&orderby=email&sortorder=asc') }}">{{ cleanLang(__('lang.email')) }}<span
                                    class="sorting-icons"><i class="ti-arrows-vertical"></i></span>
                        </th>
                        <th class="team_col_phone {{ app()->getLocale() == 'persian_br' ? 'text-right' : 'text-left' }}"><a class="js-ajax-ux-request js-list-sorting" id="sort_phone"
                                href="javascript:void(0)"
                                data-url="{{ urlResource('/team?action=sort&orderby=phone&sortorder=asc') }}">{{ cleanLang(__('lang.phone')) }}<span
                                    class="sorting-icons"><i class="ti-arrows-vertical"></i></span>
                        </th>
                        @if(config('visibility.action_super_user'))
                        <th class="team_col_last_active {{ app()->getLocale() == 'persian_br' ? 'text-right' : 'text-left' }}"><a class="js-ajax-ux-request js-list-sorting"
                        ...
                        @endif
                        @if(config('visibility.action_super_user'))
                        <th class="team_col_action {{ app()->getLocale() == 'persian_br' ? 'text-right' : 'text-left' }}"><a href="javascript:void(0)">{{ cleanLang(__('lang.action')) }}</a>
                        </th>
                        @endif
                    </tr>
                </thead>
